...
func NewBackendRepoInTransaction(ctx context.Context, backend spi.Backend, tx spi.WriteTx, now func() time.Time, progress StartupProgress) (*BackendRepo, error) {
	...
	if source == backendDomainSourceV11 {
		...
		memory, found, err := loadBackendDomainV11(ctx, tx, now)
		if err != nil {
			return nil, err
		}
		if !found {
			return nil, errors.Join(ErrAuthorityCorrupt, errors.New("detected session-tree v11 authority is missing"))
		}
		if hasUnknownEffectTurns(memory) {
			...
			changed, err := persistBackendDomainV11Changes(tx, before, memory)
			if err != nil {
				return nil, err
			}
	...
	if source == backendDomainSourceV10 {
		...
		if err := saveCompleteBackendDomainV11(tx, memory); err != nil {
			return nil, err
		}
...
func (repo *BackendRepo) VerifyCurrentStateInTransaction(ctx context.Context, tx spi.ReadTx) error {
	...
	repo.reportStartupPhase(StartupPhaseVerifying)
	memory, found, err := loadBackendDomainV11(ctx, tx, repo.now)
	if err != nil {
		return err
	}
	if !found {
	...
	if err := rejectPreV11BackendDomain(ctx, tx); err != nil {
		return err
	}
...
func rejectPreV11BackendDomain(ctx context.Context, tx spi.ReadTx) error {
	records, err := scanBackendDomainV10(ctx, tx)
	if err != nil {
		return err
	}
	if len(records) != 0 {
		return errors.Join(ErrAuthorityCorrupt, errors.New("current session-tree store retains v10 domain records"))
	}
	return rejectPreV10BackendDomain(ctx, tx)
}
...
func (repo *BackendRepo) updateDomain(ctx context.Context, mutate func(*MemoryRepo, spi.WriteTx) error) error {
	...
	err := repo.backend.Update(ctx, func(tx spi.WriteTx) error {
		...
		if _, err := persistBackendDomainV11Changes(tx, repo.domainMemory, memory); err != nil {
			return err
		}
