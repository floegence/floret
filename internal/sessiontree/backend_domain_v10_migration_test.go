...
func installV9ReferenceStore(t *testing.T, backend *migrationTestBackend) *MemoryRepo {
	t.Helper()
	...
	if err := backend.Update(t.Context(), func(tx spi.WriteTx) error { return saveCompleteBackendDomainV9(tx, memory) }); err != nil {
		t.Fatal(err)
	}
	return memory
}

func TestBackendDomainV9ToV10PreservesCanonicalBytes(t *testing.T) {
	for _, populated := range []bool{false, true} {
		t.Run(map[bool]string{false: "empty", true: "references_and_fork"}[populated], func(t *testing.T) {
			...
			memory := newMemoryRepo(time.Now)
			if populated {
				memory = installV9ReferenceStore(t, backend)
			} else if err := backend.Update(t.Context(), func(tx spi.WriteTx) error { return saveCompleteBackendDomainV9(tx, memory) }); err != nil {
				t.Fatal(err)
			}
			...
			if len(migrationTestNamespaceRecords(t, backend, backendDomainV9Namespace)) != 0 {
...
func TestBackendDomainV9ToV10RollsBackFailures(t *testing.T) {
		...
		t.Run(failure, func(t *testing.T) {
				...
				for id, entries := range memory.entries {
					for i := range entries {
						if entries[i].Type == EntryUserMessage {
							entries[i].Message.Context = []session.MessageContextItem{{Kind: "runtime", Title: "Device", Text: "local"}}
							entries[i] = PrepareEntry(entries[i])
						}
					}
					memory.entries[id] = entries
				}
				// Encode a valid v10 shape under a falsely claimed v9 envelope.
				format := backendDomainV9Format
			...
			case "future":
				...
				if err := backend.Update(ctx, func(tx spi.WriteTx) error {
					return tx.Put(format.namespace, backendDomainV9Key(backendDomainRecordManifest), raw)
				}); err != nil {
					t.Fatal(err)
				}
			}
