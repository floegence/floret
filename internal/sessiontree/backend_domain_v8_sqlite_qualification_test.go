...
func TestV9MigrationAgainstV8SQLiteCopy(t *testing.T) {
	...
	if err := backend.View(ctx, func(tx spi.ReadTx) error {
		...
		if err != nil || !found {
			return errors.Join(err, errors.New("v11 records are missing after qualification"))
		}
		return nil
	}); err != nil {
		t.Fatal(err)
...
func TestLargeV7SQLiteStartupQualification(t *testing.T) {
	...
	if err := backend.View(ctx, func(tx spi.ReadTx) error {
		...
		_, found, err := loadBackendDomainV9(ctx, tx, time.Now)
		if err != nil || !found {
			return errors.Join(err, errors.New("v11 records are missing after large-store qualification"))
		}
		return nil
	}); err != nil {
		_ = backend.Close()
