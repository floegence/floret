...
func TestBackendRepoRepairsLegacyUTF8ToolResultProjection(t *testing.T) {
	for _, withJournal := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			stateAfterFirstOpen := migrationTestNamespaceRecords(t, backend, backendDomainV12Namespace)
			if _, err := NewBackendRepo(ctx, backend, time.Now); err != nil {
				t.Fatal(err)
			}
			if !reflect.DeepEqual(migrationTestNamespaceRecords(t, backend, backendDomainV12Namespace), stateAfterFirstOpen) {
