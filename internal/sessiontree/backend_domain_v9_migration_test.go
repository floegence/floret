...
func TestBackendDomainV8ToV9MigratesResumedAndNestedForkContext(t *testing.T) {
	...
	current := migrationTestNamespaceRecords(t, backend, backendDomainV11Namespace)
	if len(current) == 0 {
	...
	if got := migrationTestNamespaceRecords(t, backend, backendDomainV11Namespace); !reflect.DeepEqual(got, current) {
...
func TestBackendDomainV9RejectsFutureRecordVersionWithoutMutation(t *testing.T) {
	...
	if err := backend.Update(t.Context(), func(tx spi.WriteTx) error {
		key := backendDomainV11Key(backendDomainRecordManifest)
		future := backendDomainV11Format
		future.version = backendDomainV11Version + 1
		...
		return tx.Put(backendDomainV11Namespace, key, encoded)
	}); err != nil {
		t.Fatal(err)
	}
	before := cloneMigrationRecords(backend.records)
