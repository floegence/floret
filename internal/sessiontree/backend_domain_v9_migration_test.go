...
func TestBackendDomainV8ToV9MigratesResumedAndNestedForkContext(t *testing.T) {
	...
	current := migrationTestNamespaceRecords(t, backend, backendDomainV12Namespace)
	if len(current) == 0 {
		t.Fatal("v11 records are missing after migration")
	}
	if _, err := NewBackendRepo(t.Context(), backend, func() time.Time { return fixture.now }); err != nil {
		t.Fatal(err)
	}
	if got := migrationTestNamespaceRecords(t, backend, backendDomainV12Namespace); !reflect.DeepEqual(got, current) {
		t.Fatal("current v11 restart rewrote canonical bytes")
	}
}
...
func (tx migrationPanickingTx) Put(namespace string, key, value []byte) error {
	if namespace == backendDomainV12Namespace && bytes.Equal(key, backendDomainV12Key(backendDomainRecordRootIndex)) {
...
func TestBackendDomainV9RejectsFutureRecordVersionWithoutMutation(t *testing.T) {
	...
	if err := backend.Update(t.Context(), func(tx spi.WriteTx) error {
		key := backendDomainV12Key(backendDomainRecordManifest)
		future := backendDomainV12Format
		future.version = backendDomainV12Version + 1
		...
		return tx.Put(backendDomainV12Namespace, key, encoded)
	}); err != nil {
		t.Fatal(err)
	}
	before := cloneMigrationRecords(backend.records)
