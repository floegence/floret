...
func TestV3ModuleAndIdentityPackageBoundary(t *testing.T) {
	module, err := os.ReadFile("go.mod")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.HasPrefix(string(module), "module github.com/floegence/floret/v3\n") {
		t.Fatal("Floret v3 must use the /v3 module path")
	}
	if _, err := os.Stat("identity"); err != nil {
		t.Fatal("Floret v3 requires the public identity package")
	}
}
	...
	for directory, packageAllowlist := range allowed {
		err := filepath.WalkDir(directory, func(path string, entry os.DirEntry, walkErr error) error {
			...
			for _, imported := range file.Imports {
				...
				const module = "github.com/floegence/floret/v3/"
