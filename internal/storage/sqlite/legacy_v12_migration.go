...
type schema12ForkPlanNode struct {
	...
	MetadataPatch       *schema12ForkDestinationMeta `json:"metadata_patch,omitempty"`
}
...
type schema13ForkPlanNode struct {
	...
	TurnIDMap           map[string]string            `json:"turn_id_map,omitempty"`
	RunIDMap            map[string]string            `json:"run_id_map,omitempty"`
	DestinationMeta     *schema12ForkDestinationMeta `json:"destination_meta,omitempty"`
}

type schema12ForkDestinationMeta struct {
...
func migrateSchemaVersion12To13(ctx context.Context, tx sqlRunner) error {
	...
	graph, err := loadSchemaVersion13ThreadAuthorityGraph(ctx, tx)
	if err != nil {
		return err
	}
...
func schema12DestinationAuthorityMatches(meta schema12ThreadAuthority, destination *schema12ForkDestinationMeta) bool {
	if destination == nil {
		return false
	}
