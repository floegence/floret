...
func NewBackendKernel(ctx context.Context, backend publicstorage.Backend, policy sessiontree.LeasePolicy, now func() time.Time) (*BackendKernel, error) {
	repo, err := sessiontree.NewBackendRepo(ctx, backend, policy, now)
	if err != nil {
		return nil, err
	}
	kernel := &BackendKernel{BackendRepo: repo}
	if err := repo.UpdateDomain(ctx, func(_ *sessiontree.MemoryRepo, tx publicstorage.WriteTx) error {
...
func loadPromptState(tx publicstorage.ReadTx) (*cache.MemoryStore, bool, error) {
...
func savePromptState(tx publicstorage.WriteTx, state *cache.MemoryStore) error {
...
func (kernel *BackendKernel) updatePrompt(ctx context.Context, mutate func(*cache.MemoryStore) error) error {
	return kernel.UpdateDomain(ctx, func(_ *sessiontree.MemoryRepo, tx publicstorage.WriteTx) error {
...
func (kernel *BackendKernel) viewPrompt(ctx context.Context, read func(*cache.MemoryStore) error) error {
	return kernel.ViewDomain(ctx, func(_ *sessiontree.MemoryRepo, tx publicstorage.ReadTx) error {
		state, found, err := loadPromptState(tx)
		if err != nil {
			return err
		}
...
func (kernel *BackendKernel) DeleteRootTree(ctx context.Context, rootThreadID string) (result sessiontree.DeleteRootTreeResult, err error) {
	err = kernel.UpdateDomain(ctx, func(memory *sessiontree.MemoryRepo, tx publicstorage.WriteTx) error {
...
func loadForkOperation(tx publicstorage.ReadTx, operationID string) (ForkOperationRecord, bool, error) {
...
func saveForkOperation(tx publicstorage.WriteTx, record ForkOperationRecord) error {
...
func (kernel *BackendKernel) ForkOperation(ctx context.Context, operationID string) (result ForkOperationRecord, err error) {
	err = kernel.ViewDomain(ctx, func(memory *sessiontree.MemoryRepo, tx publicstorage.ReadTx) error {
		var found bool
