...
func (s *Store) AppendSegment(ctx context.Context, seg promptcache.Segment) error {
	return s.withImmediate(ctx, func(tx sqlRunner) error {
		return insertSegment(ctx, tx, seg)
	})
}

func (s *Store) Segments(ctx context.Context, runID, provider, model string) ([]promptcache.Segment, error) {
	...
	defer rows.Close()
	var out []promptcache.Segment
	for rows.Next() {
		var raw string
		if err := rows.Scan(&raw); err != nil {
			return nil, err
		}
		var seg promptcache.Segment
...
func (s *Store) AppendToolset(ctx context.Context, snap promptcache.ToolsetSnapshot) error {
	return s.withImmediate(ctx, func(tx sqlRunner) error {
		return insertToolset(ctx, tx, snap)
	})
}

func (s *Store) ActiveToolset(ctx context.Context, runID, provider, model string) (promptcache.ToolsetSnapshot, bool, error) {
	var raw string
	...
	if errors.Is(err, sql.ErrNoRows) {
		return promptcache.ToolsetSnapshot{}, false, nil
	}
	if err != nil {
		return promptcache.ToolsetSnapshot{}, false, err
	}
	var snap promptcache.ToolsetSnapshot
...
func (s *Store) AppendProviderRequest(ctx context.Context, req promptcache.ProviderRequestRecord) error {
	return s.withImmediate(ctx, func(tx sqlRunner) error {
		return insertProviderRequest(ctx, tx, req)
	})
}

func (s *Store) ProviderRequests(ctx context.Context, runID string) ([]promptcache.ProviderRequestRecord, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT data_json FROM prompt_requests WHERE run_id = ? ORDER BY rowid`, runID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var out []promptcache.ProviderRequestRecord
	for rows.Next() {
		var raw string
		if err := rows.Scan(&raw); err != nil {
			return nil, err
		}
		var req promptcache.ProviderRequestRecord
...
func (s *Store) AppendProviderResponse(ctx context.Context, resp promptcache.ProviderResponseRecord) error {
	return s.withImmediate(ctx, func(tx sqlRunner) error {
		return insertProviderResponse(ctx, tx, resp)
	})
}

func (s *Store) ProviderResponses(ctx context.Context, runID string) ([]promptcache.ProviderResponseRecord, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT data_json FROM prompt_responses WHERE run_id = ? ORDER BY rowid`, runID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var out []promptcache.ProviderResponseRecord
	for rows.Next() {
		var raw string
		if err := rows.Scan(&raw); err != nil {
			return nil, err
		}
		var resp promptcache.ProviderResponseRecord
...
func (s *Store) importPromptRun(ctx context.Context, dir string) error {
	segments, err := readPromptFile[promptcache.Segment](ctx, dir, "raw_segments.jsonl")
	if err != nil {
		return err
	}
	toolsets, err := readPromptFile[promptcache.ToolsetSnapshot](ctx, dir, "toolsets.jsonl")
	if err != nil {
		return err
	}
	requests, err := readPromptFile[promptcache.ProviderRequestRecord](ctx, dir, "requests.jsonl")
	if err != nil {
		return err
	}
	responses, err := readPromptFile[promptcache.ProviderResponseRecord](ctx, dir, "responses.jsonl")
	if err != nil {
		return err
	}
	return s.withImmediate(ctx, func(tx sqlRunner) error {
...
func promptRunAlreadyImported(ctx context.Context, tx sqlRunner, segments []promptcache.Segment, toolsets []promptcache.ToolsetSnapshot, requests []promptcache.ProviderRequestRecord, responses []promptcache.ProviderResponseRecord) (bool, error) {
...
func promptRunIDs(segments []promptcache.Segment, toolsets []promptcache.ToolsetSnapshot, requests []promptcache.ProviderRequestRecord, responses []promptcache.ProviderResponseRecord) []string {
	var ids []string
	for _, seg := range segments {
...
func insertSegment(ctx context.Context, tx sqlRunner, seg promptcache.Segment) error {
...
func insertToolset(ctx context.Context, tx sqlRunner, snap promptcache.ToolsetSnapshot) error {
...
func insertProviderRequest(ctx context.Context, tx sqlRunner, req promptcache.ProviderRequestRecord) error {
...
func insertProviderResponse(ctx context.Context, tx sqlRunner, resp promptcache.ProviderResponseRecord) error {
	runID := resp.RunID
	if runID == "" {
		runID = runIDFromRequest(resp.RequestID)
	}
