...
- **状況に合わせてツールを変えられます。** `tools.Registry` に厳密なドメインツールを登録し、`runtime.ToolSurfaceProvider` で実行中の安全な時点にツール、ホスト型機能、指示、ホストコンテキストを更新できます。
- **会話を信頼できる資産にします。** `runtime.Host` がスレッド、ターン、再試行、フォーク、親が管理する子スレッド、プロバイダーにとって安全な履歴を管理します。
...
決定的な Fake Provider で永続スレッドを開始します。
...
機能を動的に変える場合は、`ToolSurfaceProvider` から `runtime.ToolSurface` を返します。製品モード、ワークスペース、権限、ホスト型機能の変更に使えます。Floret はモデルリクエストとローカルディスパッチの前にサーフェスを更新するため、古いモデル判断が新しい製品ポリシーを静かにすり抜けることはありません。
