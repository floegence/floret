...
if [[ $# -ne 1 || $1 != v7.* ]]; then
	printf 'usage: scripts/check_published_release_adoption.sh <exact-v7-tag>\n' >&2
  exit 1
fi
readonly tag=$1
readonly adoption_root=$(mktemp -d "${TMPDIR:-/tmp}/floret-v7-published.XXXXXX")
cleanup() {
...
go mod init example.com/floret-v7-published-adoption
cp "${repository_root}/scripts/testdata/v7_adoption_test.go" adoption_test.go
