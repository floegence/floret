...
if [[ $# -ne 1 || $1 != v2.* ]]; then
  printf 'usage: scripts/check_published_release_adoption.sh <exact-v2-tag>\n' >&2
  exit 1
fi
readonly tag=$1
readonly adoption_root=$(mktemp -d "${TMPDIR:-/tmp}/floret-v2-published.XXXXXX")
cleanup() {
...
go mod init example.com/floret-v2-published-adoption
