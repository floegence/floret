...
go mod init example.com/floret-v7-api-baseline >/dev/null
go get "${module_path}/...@${baseline_tag}" >/dev/null
go run "${apidiff_package}" -m -w "${compatibility_root}/v7.api" "${module_path}"
cd - >/dev/null

go run "${apidiff_package}" -m -w "${compatibility_root}/head.api" "${module_path}"
readonly incompatible=$(go run "${apidiff_package}" -m -incompatible "${compatibility_root}/v7.api" "${compatibility_root}/head.api")
if [[ -n ${incompatible} ]]; then
  printf 'v7 API compatibility: incompatible changes from %s to HEAD\n%s\n' "${baseline_tag}" "${incompatible}" >&2
  exit 1
fi
printf 'v7 API compatibility: %s to HEAD has no incompatible changes\n' "${baseline_tag}"
