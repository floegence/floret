...
export GOWORK=off
go test ./internal/architecture -run TestV4PublicAPIBaselineIsCurrent -count=1

if ! git rev-parse --quiet --verify "refs/tags/${baseline_tag}" >/dev/null; then
  printf 'v4 API compatibility: %s is not tagged; the designed go/types baseline matches\n' "${baseline_tag}"
  exit 0
fi
...
go mod init example.com/floret-v4-api-baseline >/dev/null
...
if [[ -n ${incompatible} ]]; then
  printf 'v4 API compatibility: incompatible changes from %s to HEAD\n%s\n' "${baseline_tag}" "${incompatible}" >&2
  exit 1
fi
printf 'v4 API compatibility: %s to HEAD has no incompatible changes\n' "${baseline_tag}"
