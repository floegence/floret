...
func (m ModelMessage) Validate() error {
	...
	case ModelMessageRoleSystem, ModelMessageRoleUser:
		if strings.TrimSpace(m.Text) == "" {
			return fmt.Errorf("%s model message requires text", m.Role)
		}
		if m.Reasoning != "" || len(m.ToolCalls) > 0 || m.ToolResult != nil {
			return fmt.Errorf("%s model message contains assistant or tool fields", m.Role)
		}
	case ModelMessageRoleAssistant:
		if m.ToolResult != nil {
			return errors.New("assistant model message cannot contain a tool result")
		}
...
func runtimeModelMessages(messages []session.Message) ([]ModelMessage, error) {
	...
	for index := 0; index < len(messages); {
		...
		case session.System, session.User:
			projected := ModelMessage{Role: ModelMessageRole(msg.Role), Text: msg.Content}
