...
type subAgentManagerView struct {
	parent  *Thread
	agent   *Agent
...
func (thread *Thread) setTitle(ctx context.Context, command SetThreadTitleCommand) (SetThreadTitleResult, error) {
	...
	err = host.applyThreadMutation(ctx, thread.id, func() error {
		record, replayed, reserveErr := host.reserveBoundMutation(ctx, "set_thread_title", thread.id, command.LogicalRequestID, fingerprint)
		if reserveErr != nil {
			return reserveErr
		}
		if replayed && record.State == requestStateCommitted {
			if decodeErr := decodeLedgerResult(record, &out); decodeErr != nil {
				return decodeErr
			}
			out.Receipt = receiptFromRecord(record, true)
			return nil
		}
		editor, editorErr := host.threadTitleEditor(ctx, thread.id)
		if editorErr != nil {
			return editorErr
		}
		snapshot, setErr := editor.Set(ctx, command.Title)
		if setErr != nil {
			return setErr
		}
		out = SetThreadTitleResult{Thread: snapshot}
		if commitErr := host.commitMutationResult(context.WithoutCancel(ctx), thread, &record, &out); commitErr != nil {
			return commitErr
		}
...
func (thread *Thread) turns(agent *Agent) (*turnExecutorView, error) {
	if thread == nil || thread.host == nil {
		return nil, errors.New("thread is required")
	}
	if err := thread.host.available(); err != nil {
	...
	return &turnExecutorView{thread: thread, agent: agent}, nil
}
...
func (thread *Thread) subAgents(ctx context.Context, agent *Agent) (*subAgentManagerView, error) {
	if thread == nil || thread.host == nil {
		return nil, errors.New("thread is required")
	}
	if agent == nil {
	...
	return &subAgentManagerView{parent: thread, agent: agent, manager: manager, reader: reader}, nil
}
...
func (subAgents *subAgentManagerView) List(ctx context.Context) ([]SubAgentSnapshot, error) {
...
func (subAgents *subAgentManagerView) SpawnSubAgent(ctx context.Context, command SpawnSubAgentCommand) (SpawnSubAgentResult, error) {
...
func (subAgents *subAgentManagerView) SendSubAgentMessage(ctx context.Context, command SendSubAgentMessageCommand) (SendSubAgentMessageResult, error) {
...
func (subAgents *subAgentManagerView) InterruptSubAgent(ctx context.Context, command InterruptSubAgentCommand) (InterruptSubAgentResult, error) {
...
func (subAgents *subAgentManagerView) WaitSubAgents(ctx context.Context, command WaitSubAgentsCommand) (WaitSubAgentsResult, error) {
...
func (subAgents *subAgentManagerView) CloseSubAgent(ctx context.Context, command CloseSubAgentCommand) (CloseSubAgentResult, error) {
	if subAgents == nil || subAgents.parent == nil || subAgents.parent.host == nil || subAgents.manager == nil {
...
func (subAgents *subAgentManagerView) sendSubAgentInput(ctx context.Context, operation string, requestID identity.LogicalRequestID, childThreadID identity.ThreadID, input TurnInput, labels RunLabels, interrupt bool) (SendSubAgentMessageResult, error) {
	if subAgents == nil || subAgents.parent == nil || subAgents.parent.host == nil || subAgents.manager == nil {
...
func (subAgents *subAgentManagerView) activateCommittedChild(ctx context.Context, childThreadID identity.ThreadID) error {
...
func (turns *turnExecutorView) StartTurn(ctx context.Context, command StartTurnCommand) (StartTurnResult, error) {
	...
	var runner *turnRunnerHandle
	committed := false
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		var reserveErr error
		record, replayed, reserveErr = host.reserveStartTurn(ctx, turns.thread.id, command.LogicalRequestID, fingerprint, &plan)
		if reserveErr != nil {
			return reserveErr
		}
		...
		var runnerErr error
		runner, runnerErr = host.turnRunner(ctx, turns.thread.id, &executionAgent)
		return runnerErr
	})
	...
	if result.TurnID != "" {
		...
		commitErr := host.applyThreadMutation(commitCtx, turns.thread.id, func() error {
			view, snapshotErr := turns.thread.snapshot(commitCtx)
			if snapshotErr != nil {
				return snapshotErr
			}
			record.Revision = view.Revision
			record.State = requestStateCommitted
			return host.commitRequest(commitCtx, record)
		})
		if commitErr != nil {
...
func (turns *turnExecutorView) AdmitTurn(ctx context.Context, command StartTurnCommand) (AdmitTurnResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
	...
	unlockExecution := host.turnExecutions.lock(turns.thread.id.String())
	defer unlockExecution()
	var out AdmitTurnResult
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		record, requestReplayed, reserveErr := host.reserveStartTurn(ctx, turns.thread.id, command.LogicalRequestID, fingerprint, &plan)
		if reserveErr != nil {
			return reserveErr
		}
		executionAgent := *turns.agent
		...
		revision, revisionErr := host.currentThreadRevision(ctx, record.ThreadID)
		if revisionErr != nil {
			return revisionErr
		}
		receipt := TurnAdmissionReceipt{
...
func (execution *turnAdmissionExecution) turns() *turnExecutorView {
	if execution == nil {
		return nil
	}
	return &turnExecutorView{thread: execution.thread, agent: execution.agent}
}
...
func (turns *turnExecutorView) ExecuteAdmission(ctx context.Context, receipt TurnAdmissionReceipt, executionContext ExecutionContext) (StartTurnResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
		return StartTurnResult{}, errors.New("turn authority is required")
	}
	host := turns.thread.host
	...
	var record requestLedgerRecord
	err := host.applyThreadMutation(ctx, turns.thread.id, func() error {
		loaded, found, loadErr := host.loadRequest(ctx, "start_turn", turns.thread.id.String(), receipt.LogicalRequestID)
		if loadErr != nil {
			return loadErr
		}
		if !found {
			return ErrExecutionPlanUnavailable
		}
		if validationErr := validateAdmittedTurnRecord(loaded, receipt); validationErr != nil {
...
func (turns *turnExecutorView) executeAdmissionRecord(ctx context.Context, record requestLedgerRecord, receipt TurnAdmissionReceipt, executionContext ExecutionContext, replayed bool) (StartTurnResult, error) {
...
func (turns *turnExecutorView) RetryTurn(ctx context.Context, command RetryTurnCommand) (RetryTurnResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
	...
	var record requestLedgerRecord
	var replayed bool
	var out RetryTurnResult
	var runner *turnRunnerHandle
	committed := false
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		var reserveErr error
		record, replayed, reserveErr = host.reserveTurnMutation(ctx, "retry_turn", turns.thread.id, command.LogicalRequestID, fingerprint)
		if reserveErr != nil {
			return reserveErr
		}
...
func (turns *turnExecutorView) ContinuePendingTool(ctx context.Context, command ContinuePendingToolCommand) (ContinuePendingToolResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
	...
	var replayed bool
	var runner *turnRunnerHandle
	var replayedOut *ContinuePendingToolResult
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		var reserveErr error
		record, replayed, reserveErr = host.reserveTurnMutation(ctx, "continue_pending_tool", turns.thread.id, command.LogicalRequestID, fingerprint)
		if reserveErr != nil {
			return reserveErr
		}
		if replayed && record.State == requestStateCommitted {
	...
	if completion.ThreadID != "" {
		commitCtx := context.WithoutCancel(ctx)
		commitErr := host.applyThreadMutation(commitCtx, turns.thread.id, func() error {
			return host.commitMutationResult(commitCtx, turns.thread, &record, &out)
		})
		if commitErr != nil {
			return out, errors.Join(runErr, commitErr)
		}
...
// provider loop.
func (turns *turnExecutorView) RecordPendingToolOutcome(ctx context.Context, command RecordPendingToolOutcomeCommand) (RecordPendingToolOutcomeResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
	...
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		record, replayed, reserveErr := host.reserveBoundMutation(ctx, "record_pending_tool_outcome", turns.thread.id, command.LogicalRequestID, fingerprint)
		if reserveErr != nil {
			return reserveErr
		}
		if replayed && record.State == requestStateCommitted {
		...
		runner, runnerErr := host.turnRunner(ctx, turns.thread.id, turns.agent)
		if runnerErr != nil {
			return runnerErr
		}
		outcome, settleErr := runner.SettlePendingTool(ctx, activePendingToolSettlement{Target: command.Target, Status: command.Status, Summary: command.Summary, Output: command.Output, Activity: command.Activity})
		if settleErr != nil {
			return settleErr
		}
		out = RecordPendingToolOutcomeResult{Outcome: outcome}
		if commitErr := host.commitMutationResult(context.WithoutCancel(ctx), turns.thread, &record, &out); commitErr != nil {
			return commitErr
		}
		out.Receipt = receiptFromRecord(record, replayed)
		return nil
...
func (turns *turnExecutorView) ResolveApproval(ctx context.Context, command ResolveApprovalCommand) (ResolveApprovalCommandResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
	...
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		record, replayed, reserveErr := host.reserveBoundMutation(ctx, "resolve_approval", turns.thread.id, command.LogicalRequestID, fingerprint)
		if reserveErr != nil {
			return reserveErr
		}
		if replayed && record.State == requestStateCommitted {
...
func (turns *turnExecutorView) UpdateTodos(ctx context.Context, command UpdateTodosCommand) (UpdateTodosResult, error) {
	if turns == nil || turns.thread == nil || turns.thread.host == nil || turns.agent == nil {
	...
	err = host.applyThreadMutation(ctx, turns.thread.id, func() error {
		record, replayed, reserveErr := host.reserveBoundMutation(ctx, "update_todos", turns.thread.id, command.LogicalRequestID, fingerprint)
		if reserveErr != nil {
			return reserveErr
		}
		if replayed && record.State == requestStateCommitted {
		...
		if replayed {
			reader, readerErr := host.threadReader(ctx, turns.thread.id)
			if readerErr != nil {
				return readerErr
			}
			state, stateErr := reader.ReadAgentTodos(ctx)
			if stateErr != nil {
				return stateErr
			}
		...
		state, stateErr := runner.UpdateAgentTodos(ctx, agentTodoUpdateRequest{ExpectedVersion: command.ExpectedVersion, Items: command.Items, TurnID: command.TurnID, RunID: command.RunID, ToolCallID: command.ToolCallID})
		if stateErr != nil {
			return stateErr
		}
		out = UpdateTodosResult{State: state}
...
func (host *Host) finishShutdown() {
	if host.threadActors != nil {
		host.threadActors.close()
	}
	storeErr := host.store.Close()
	err := errors.Join(storeErr, host.backend.Close())
...
func (host *Host) publishSubscriptionEvent(event Event) {
	...
	if !transientRuntimeEvent(event.Type) && event.Stream == nil && event.Type != observation.EventTypeProviderRequest {
		return
	}
	actor := host.threadActors.actor(event.ThreadID.String())
	_ = actor.apply(context.Background(), func() error {
		if !actor.acceptLiveEvent(event) || !transientRuntimeEvent(event.Type) {
			return nil
		}
...
func (host *Host) reserveStartTurn(ctx context.Context, threadID identity.ThreadID, requestID identity.LogicalRequestID, fingerprint string, plan *turnExecutionPlan) (requestLedgerRecord, bool, error) {
