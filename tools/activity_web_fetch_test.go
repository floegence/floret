...
func TestWebFetchActivityRoundTripCloneMergeAndFinalize(t *testing.T) {
	...
	if initial.Payload.(WebFetchActivityPayload).URL != "https://example.com/start" || initial.Payload.(WebFetchActivityPayload).Error != nil {
	...
	if mergedPayload.URL != "https://example.com/start" || mergedPayload.FinalURL != "https://example.com/final" || mergedPayload.Status != "error" || mergedPayload.Error == nil || !mergedPayload.Truncated {
...
func TestWebFetchActivityValidatesBoundedMetadata(t *testing.T) {
	...
	valid := ActivityPresentation{
		...
		Payload: WebFetchActivityPayload{
			...
			StatusCode: 200, ContentType: "text/plain", Format: "text", BytesRead: 10,
		},
	}
