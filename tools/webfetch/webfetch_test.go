...
func TestDefinitionKeepsFixedPublicContract(t *testing.T) {
	...
	if requestTimeout != 30*time.Second || maxRedirects != 5 || maxBodyBytes != 5<<20 {
		t.Fatalf("fixed limits changed: timeout=%s redirects=%d bytes=%d", requestTimeout, maxRedirects, maxBodyBytes)
	}
}
...
func TestFetchConvertsHTMLAndKeepsOnlyUntrustedPageData(t *testing.T) {
	...
	if !ok || activity.Status != "success" || activity.URL == "" || activity.ContentType == "" || activity.Error != nil {
...
func TestFetchReturnsReadableHTTPErrorResult(t *testing.T) {
	...
	for _, status := range []int{http.StatusNotFound, http.StatusBadGateway} {
		...
		t.Run(http.StatusText(status), func(t *testing.T) {
			...
			if !ok || payload.Status != "error" || payload.StatusCode != status || payload.Error == nil {
				t.Fatalf("activity = %#v", result.Activity)
			}
		})
	}
