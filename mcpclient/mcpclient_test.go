...
func TestStdioServerListsAndCallsTools(t *testing.T) {
	...
	manager := mcpclient.NewManager(mcpclient.Options{})
	err := manager.Start(ctx, []mcpclient.ServerConfig{{
		Name:           "context7",
	...
	result := reg.Run(context.Background(), provider.ToolCall{ID: "call-1", Name: "mcp__context7__search", Args: `{"query":"floret"}`}, allowAll)
...
func TestStreamableHTTPServerListsAndCallsTools(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch req["method"] {
		case "initialize":
			writeRPC(w, id, map[string]any{"protocolVersion": mcpclient.ProtocolVersion, "instructions": "test"})
	...
	manager := mcpclient.NewManager(mcpclient.Options{})
	err := manager.Start(context.Background(), []mcpclient.ServerConfig{{
		Name:      "docs",
	...
	result := reg.Run(context.Background(), provider.ToolCall{ID: "call-1", Name: "mcp__docs__lookup", Args: `{"id":"abc"}`}, allowAll)
...
func TestRefreshUpdatesToolSnapshotBetweenTurns(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "initialize", "notifications/initialized":
			writeRPC(w, id, map[string]any{"protocolVersion": mcpclient.ProtocolVersion})
		case "tools/list":
			listCalls++
	...
	manager := mcpclient.NewManager(mcpclient.Options{})
	if err := manager.Start(context.Background(), []mcpclient.ServerConfig{{Name: "refresh", Transport: mcpclient.TransportStreamableHTTP, URL: server.URL, Enabled: true}}); err != nil {
		t.Fatal(err)
	}
	...
	if !slices.ContainsFunc(defs, func(def provider.ToolDefinition) bool { return def.Name == "mcp__refresh__two" }) {
		t.Fatalf("refreshed tool not registered: %#v", defs)
	}
}

func TestOptionalAndRequiredServerFailures(t *testing.T) {
	optional := mcpclient.NewManager(mcpclient.Options{})
	err := optional.Start(context.Background(), []mcpclient.ServerConfig{{
		Name:      "missing",
		Transport: mcpclient.TransportStdio,
	...
	if got := optional.Snapshots(); len(got) != 1 || got[0].Status != mcpclient.StatusFailed {
		t.Fatalf("optional snapshots = %#v", got)
	}

	required := mcpclient.NewManager(mcpclient.Options{})
	err = required.Start(context.Background(), []mcpclient.ServerConfig{{
		Name:      "missing",
		Transport: mcpclient.TransportStdio,
		Command:   "/definitely/not/here",
		Enabled:   true,
...
func TestMissingInputSchemaIsRejected(t *testing.T) {
	...
	defer server.Close()
	manager := mcpclient.NewManager(mcpclient.Options{})
	err := manager.Start(context.Background(), []mcpclient.ServerConfig{{Name: "bad", Transport: mcpclient.TransportStreamableHTTP, URL: server.URL, Enabled: true, Required: true}})
...
func TestServerNameCollisionAndUnsafeToolNamesAreRejected(t *testing.T) {
	...
	err = manager.Start(context.Background(), []mcpclient.ServerConfig{{Name: "bad-name", Enabled: false}})
	...
	defer server.Close()
	err = manager.Start(context.Background(), []mcpclient.ServerConfig{{Name: "safe", Transport: mcpclient.TransportStreamableHTTP, URL: server.URL, Enabled: true, Required: true}})
...
func TestInitializedNotificationFailureFailsStartup(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		writeRPC(w, req["id"], map[string]any{"protocolVersion": mcpclient.ProtocolVersion})
	}))
	defer server.Close()
	manager := mcpclient.NewManager(mcpclient.Options{})
	err := manager.Start(context.Background(), []mcpclient.ServerConfig{{Name: "bad", Transport: mcpclient.TransportStreamableHTTP, URL: server.URL, Enabled: true, Required: true}})
...
func TestPermissionDenyBlocksMCPToolCall(t *testing.T) {
	...
	manager := mcpclient.NewManager(mcpclient.Options{})
	if err := manager.Start(context.Background(), []mcpclient.ServerConfig{{
		Name:           "context7",
		Transport:      mcpclient.TransportStdio,
		Command:        os.Args[0],
	...
	result := reg.Run(context.Background(), provider.ToolCall{ID: "call-1", Name: "mcp__context7__search", Args: `{"query":"floret"}`}, denyAll)
...
func runFakeStdioMCP(t *testing.T) {
	...
	for {
		...
		switch req["method"] {
		case "initialize":
			result = map[string]any{"protocolVersion": mcpclient.ProtocolVersion, "instructions": "fake stdio"}
		case "notifications/initialized":
			continue
		case "tools/list":
