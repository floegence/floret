...
func TestSkillToolReadsFullSkillMarkdown(t *testing.T) {
	...
	result := reg.Run(context.Background(), provider.ToolCall{ID: "call-1", Name: "skill", Args: `{"name":"pdf"}`}, nil)
...
func TestSkillToolRejectsUnknownSkill(t *testing.T) {
	...
	reg := tools.NewRegistry(tool)
	result := reg.Run(context.Background(), provider.ToolCall{ID: "call-1", Name: "skill", Args: `{"name":"other"}`}, nil)
